...
export class CareersService {
  ...
  async getApplications(queries: string[] = []) {
    try {
      const allQueries = [Query.select(["*", "vacancy.title"]), ...queries];
      return await databases.listDocuments(
        conf.appwriteDatabaseId,
        conf.appwriteJobApplicationsTableId,
        allQueries,
      ...
      console.error(
        "CareersService :: getApplicationsWithVacancyName :: error",
        error,
      );
      throw error;
    }
  ...
  async deleteApplication(applicationId: string) {
    try {
      ...
      console.error("CareersService :: deleteApplication :: error", error);
      throw error;
    }
  }
  ...
  getResumeDownloadUrl(fileId: string) {
    return storage.getFileDownload(conf.appwriteBucketID, fileId);
  }
}
